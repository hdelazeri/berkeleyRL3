...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        best_action_value = self.getQValue(state, legal_actions[0])
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        actual_value = (1 - self.alpha) * self.getQValue(state, action)
        next_state_value = self.discount * self.getValue(nextState)
        increment = self.alpha * (reward + next_state_value)

        new_value = actual_value + increment
